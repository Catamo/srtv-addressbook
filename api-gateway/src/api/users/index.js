...
module.exports = (app, container) => {
  ...
  app.post('/users', (req, res, next) => {
    amqpClient.sendRPCMessage(amqpChannel, JSON.stringify(req.body), amqpQueues.userRegisterQueue)
      .then(msg => {
  ...
  app.post('/users/:id/contacts', passport.authenticate('jwt', {session: false}), (req, res, next) => {
    let contact = JSON.stringify(req.body)
    contact.ofUser = req.params.id
    console.log('contacts- ', contact, req.params.id)
    amqpClient.sendRPCMessage(amqpChannel, contact, amqpQueues.contactCreateQueue)
      .then(msg => {
        const result = JSON.parse(msg)
        res.status(status.CREATED).json(result)
      })
